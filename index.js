...
app.post('/transpileText', (req, res, next) => {
   ...
   /* fs.writeFile('./hardhat-project/contracts/HelloWorld.sol' ,  file, function(err,data){
    ...
    exec(`cd ./hardhat-project/ && (npm install --save-dev hardhat) && (npm install dotenv --save) && (npm install --save-dev @nomiclabs/hardhat-ethers "ethers@^5.0.0") && (npx hardhat compile) && (npx run ./scripts/interact.js) `,
        function (error, stdout, stderr) {
            console.log('stdout: ' + stdout);
            console.log('stderr: ' + stderr);
            if (error !== null) {
